...
<section class="section-hero overlay inner-page bg-image" style="background-color:#d6cdcc;" id="home-section">
    <div class="container">
        <div class="row">
            <div class="col-md-7">
                <h3 class="text-white font-weight-bold">Employer Login</h3>
...
if (isset($_POST['btn_submit']) && $count == 0) {
    ...
    $select = "SELECT email, password FROM tbl_registration_employers WHERE email = '$username' AND password = '$password'";
    $query = mysqli_query($con, $select);
    $row = mysqli_fetch_array($query);

    if ($username == $row['email'] && $password == $row['password']) {
        $_SESSION['employer'] = $row['email'];
